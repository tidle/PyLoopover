# This is a short module that describes the version of PyLoopover
# It is changed when a new version is released
# Version numbers work like this:
# 0.1 : pre release version 1
# 1.1 : version 1, subversion 2 (1.0 was the first sub-version)
# 1.1.1 : no difference to the user from version 1.1, but contains bugfixes, or archatecture changes
# 2.3+ : development version after 2.3. Could become 2.4 or 3.0
version = "1.2.1"
